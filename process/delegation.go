...
func (process *Process) delegate(u user.User) {
	...
	delegateeIdx, ok := epochDelegations[u.GetIndex()]
	if !ok {
		return
	}
	delegateeAddress := process.users[delegateeIdx].GetAddress()
...
func (process *Process) undelegate(u user.User) {
	...
	hash, err := u.SendTransaction(UndelegateTx, nil, 0, 0, nil)
	if err != nil {
...
func (process *Process) delegateTo(u user.User, delegateeAddress string) {
	...
	hash, err := u.SendTransaction(DelegateTx, &delegateeAddress, 0, 0, nil)
	if err != nil {
...
func (process *Process) killDelegators(u user.User) {
	...
	killDelegators, ok := epochKillDelegators[u.GetIndex()]
	if !ok {
		return
	}
	...
	for _, delegatorToKill := range killDelegators {
		func(delegatorToKill int) {
			to := process.users[delegatorToKill].GetAddress()
			hash, err := u.SendTransaction(KillDelegatorTx, &to, 0, 0, nil)
			if err != nil {
